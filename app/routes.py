...
def summary(name: str, stroke: str, db: Session = Depends(get_db)):
    ...
    pb_seconds = min(
        (s for r in rows if not is_winter_short_course(r["m"]) 
         for s in [parse_seconds(r["r"])] if s),
        default=None
    )
    trend = [{"year": r["y"], "seconds": parse_seconds(r["r"])} for r in rows if parse_seconds(r["r"])]
    return {
        "analysis":{"meetCount":len(items),"avg_seconds": (sum(secs)/len(secs)) if secs else None,"pb_seconds":pb_seconds},
        "trend":{"points":trend},
        "items":items
    }
...
def rank_api(name: str, stroke: str, db: Session = Depends(get_db)):
    """
    對手池改為：不論組別內容是數字或文字，**一律要求組別完全相同**（含空字串）。
    """
    pat = f"%{stroke.strip()}%"

    # 先找出輸入選手在該泳姿＋距離的所有 (年,賽,項,組)
    q_meets = f"""
    ...
    for m in base:
        ...
        for r in rows:
            opponents[r[0]] = {"name": r[0]}

    if not opponents:
        return {"denominator":0,"rank":None,"top":[],"leaderTrend":[]}

    # PB（同泳姿＋距離、剔除冬短）
    def best_of(player:str):
        q=f"""SELECT "年份"::text AS y,"賽事名稱"::text AS m,"成績"::text AS r
              FROM {TABLE} WHERE "姓名"=:p AND "項目" ILIKE :pat
              ORDER BY "年份" ASC"""
    ...
    for r in db.execute(text(q),{"p":leader["name"],"pat":pat}).mappings():
        s=parse_seconds(r["r"])
        if s:
            leader_trend.append({"year":r["y"],"seconds":s,"meet":r["m"]})
