...
app/routes.py
v2025.09.16.03
- CORS: allow_origins configurable; default "*" with allow_credentials=False to避免瀏覽器阻擋
- 例外保護：summary / rank 包 try/except，回傳 JSON 錯誤訊息，以利前端顯示
"""
from __future__ import annotations
import os, re, sqlite3, traceback
...
app = FastAPI(title="Swim API", version="v2025.09.16.03")
router = APIRouter()

# CORS（建議在 Render 設定 API_ALLOW_ORIGINS 為你的前端網域，例如 https://your-frontend.app）
allow = os.getenv("API_ALLOW_ORIGINS", "*")
app.add_middleware(
    CORSMiddleware,
    allow_origins=[o.strip() for o in allow.split(",")] if allow != "*" else ["*"],
    allow_credentials=False,  # 若需要 cookies，必須把 allow_origins 換成明確網域並改 True
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def health():
    ...
    # 試著連 DB 並查一下表是否存在
    try:
        _ = q("SELECT 1 FROM swimming_scores LIMIT 1")
        has_table = True
    except Exception:
        has_table = False
    return {"ok": "true", "db": DB_PATH, "db_exists": ok, "has_table": has_table}

@router.get("/summary")
def summary(name: str, stroke: str, limit: int = 200, cursor: int = 0):
    try:
        ...
        return JSONResponse(status_code=500, content={"error":"summary_failed","message":str(e),"trace":traceback.format_exc()})

@router.get("/rank")
def rank(name:str, stroke:str):
    try:
        ...
        return JSONResponse(status_code=500, content={"error":"rank_failed","message":str(e),"trace":traceback.format_exc()})
